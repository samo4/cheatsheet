...
- **5.2.1** — Asymptotically stable: all $\operatorname{Re}\lambda_i < 0$ — every mode decays, so the state converges to the equilibrium and the transients die. Marginal: simple poles on the imaginary axis — bounded but not decaying: a sustained oscillation (imaginary pair, phase-portrait centre) or a constant that stays put (pole at the origin). A repeated axis pole grows like $t$ — that is unstable, not marginal.^[For *poles* of a SISO transfer function (minimal realization) this is exactly right. For *eigenvalues* of $\mathbf{A}$ it is only a rule of thumb: a repeated eigenvalue on the axis is harmless when it has as many independent eigenvectors as its multiplicity (only $1\times1$ Jordan blocks), e.g. $\dot{\vec{x}} = \mathbf{0}_{2\times2}\,\vec{x}$. See $\Phi$ via the Jordan form in the State-space chapter.]
